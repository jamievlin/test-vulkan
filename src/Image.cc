...
VkFormat findSuitableFormat(
    VkPhysicalDevice const& dev, std::vector<VkFormat> const& formats,
    VkImageTiling const& tilingMode, VkFormatFeatureFlags const& flags
)
{
    for (auto const& fmt : formats)
    {
        ...
        if (tilingMode == VK_IMAGE_TILING_LINEAR && (prop.linearTilingFeatures & flags) == flags)
        {
            return fmt;
        }
        else if (tilingMode == VK_IMAGE_TILING_OPTIMAL && (prop.optimalTilingFeatures & flags) == flags)
        {
            return fmt;
        }
    }
...
{
    std::vector const fmts(depthFormats, depthFormats + depthFormatCount);
    return findSuitableFormat(
        dev, fmts, VK_IMAGE_TILING_OPTIMAL, VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT
    );
}
...
Image::Image(
    ...
    : AVkGraphicsBase(logicalDev), allocator(allocator), size(size.first, size.second, 1)
{
    auto const& [width, height] = size;
    CHECK_VK_SUCCESS(
        createImage(
            imgFormat, usage, memoryFlags, queueTransfers, VK_IMAGE_TYPE_2D, tiling, mipLevels,
            initialLayout
        ),
    ...
    {
        CHECK_VK_SUCCESS(
            createSampler(samplerCreateInfo.value()), ErrorMessages::FAILED_CANNOT_CREATE_SAMPLER
        );
    }
}
    ...
    : AVkGraphicsBase(logicalDev), allocator(allocator), size(std::move(size))
{
    CHECK_VK_SUCCESS(
        createImage(
            imgFormat, usage, memoryFlags, queueTransfers, VK_IMAGE_TYPE_3D, tiling, mipLevels,
            initialLayout
        ),
        ErrorMessages::FAILED_CANNOT_CREATE_IMAGE
    );
...
VkResult Image::createImage(
    VkFormat const& imgFormat, VkImageUsageFlags const& usage,
    VkMemoryPropertyFlags const& memoryFlags, optional<std::set<uint32_t>> const& queues,
    VkImageType const& imgType, VkImageTiling const& tiling, uint32_t const& mipLevels,
...
Image::Image(Image&& im) noexcept
    : AVkGraphicsBase(std::move(im)), img(std::move(im.img)), imgView(std::move(im.imgView)),
      baseSampler(std::move(im.baseSampler)), allocation(std::move(im.allocation)),
      allocator(std::move(im.allocator)), size(std::move(im.size))
{
}
...
VkResult Image::createBaseImageView(
    VkFormat const& format, VkImageViewType const& viewType, VkImageAspectFlags const& aspectFlags
)
{
...
void Image::cmdTransitionLayout(
    ...
    VkCommandBuffer& cmdBuffer, VkImageAspectFlags const& aspectFlags
) const
{
...
void Image::cmdCopyFromBuffer(
    Buffers::Buffer const& srcBuffer, VkImageLayout const& layout, VkCommandBuffer& cmdBuffer
)
{
    ...
    vkCmdCopyBufferToImage(cmdBuffer, srcBuffer.vertexBuffer, img, layout, 1, &copyRegion);
}
