...
Mesh::Mesh(
    ...
    : AVkGraphicsBase(logicalDev), allocator(allocator), physDev(physDev)
{
...
)
{
    ...
    auto stg_ptr = std::make_shared<Buffers::StagingBuffer>(
        ...
        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, transferQueues
    );
...
Mesh::Mesh(Mesh&& mesh) noexcept
    : AVkGraphicsBase(std::move(mesh)), buf(std::move(mesh.buf)), verts(std::move(mesh.verts)),
      indices(std::move(mesh.indices)), allocator(std::move(mesh.allocator)),
